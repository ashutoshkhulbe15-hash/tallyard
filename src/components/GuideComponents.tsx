...
export function ComparisonTable({
  ...
  return (
    <div className="my-8 -mx-4 md:mx-0 overflow-x-auto">
      <table className="w-full min-w-[560px] bg-surface border border-line rounded-lg overflow-hidden text-sm">
        <thead>
          <tr className="bg-bg-warm border-b border-line">
            <th className="text-left font-semibold text-ink-faint uppercase tracking-wider text-[11px] px-4 py-3 w-1/4">
              &nbsp;
            </th>
            {columns.map((col, i) => (
              <th
        ...
        <tbody>
          {rows.map((row, i) => (
            ...
            >
              <td className="text-xs font-semibold text-ink-faint uppercase tracking-wider px-4 py-3 align-top">
                {row.label}
              </td>
              {row.values.map((val, j) => (
                <td
                  key={j}
                  className={`text-sm text-ink px-4 py-3 align-top ${
